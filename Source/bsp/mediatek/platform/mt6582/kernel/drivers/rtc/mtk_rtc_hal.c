...
/*lenovo-sw weiweij modified for rtc hour reg write error*/
#if 1
	if(addr == RTC_AL_HOU)
	{
		hal_rtc_xinfo("~~RTC_AL_HOU will be modified ,original value is 0x%x,Target Value is 0x%x \n",rtc_read(RTC_AL_HOU),data);
		data=((rtc_read(RTC_AL_HOU) & (RTC_NEW_SPARE_FG_MASK))|(data&0x00ff));
		hal_rtc_xinfo("~~Target Value after modified is 0x%x \n",data);
	}
#endif
/*lenovo-sw weiweij modified for rtc hour reg write error end*/
