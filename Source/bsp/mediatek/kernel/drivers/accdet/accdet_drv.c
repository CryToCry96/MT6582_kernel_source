...
/* lenovo-sw zhouwl, 2013-07-10, compatible accdet interrupt */
    ...
    if (cur_eint_state == EINT_PIN_PLUG_IN) {
		...
		#endif
		/* lenovo-sw zhouwl, 2013-06-27, add for sofina dvt1 hw, add a pa for earphone */
		#ifdef LENOVO_PROJECT_SOFINA
...
/* lenovo-sw zhouwl, 2013-07-10, compatible accdet interrupt */
#ifdef LENOVO_COMPATIBLE_DVT_EARPHONE
void accdet_exteint_func(void)
{
	int ret=0;
	if(cur_eint_state ==  EINT_PIN_PLUG_IN ) 
	{
	 accdet_exteint_polarity_reverse_plugout();
	 //ACCDET_DEBUG("[Accdet]EINT func :plug-out\n");
	 cur_eint_state = EINT_PIN_PLUG_OUT;
	} else {
	 accdet_exteint_polarity_reverse_plugin();
	 cur_eint_state = EINT_PIN_PLUG_IN;
